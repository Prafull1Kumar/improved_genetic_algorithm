...
pset_csa = gp.PrimitiveSet("MAIN", arity=2)
...
pset_st_gp = gp.PrimitiveSet("MAIN", arity=2)
    ...
    for x in range(-10, 11):
        for y in range(-10, 11):
            ...
            try:
                if func(x,y) is None:
                    raise TypeError
                error += abs(func(x,y) - (3*x*x*x +4*y*y*x+11*y*y)) # Fitness function
            except:
                error += 100
    return error,
...
for run in range(num_runs):
    ...
    # plt.title("3*x*x*x+4*y*y*x+11*y*y")
