...
def evaluate_csa(individual):
    ...
    for x in range(-10, 11):
            try:
                if func(x) is None:
                    raise TypeError
                error += abs(func(x) - (x)) # Fitness function
            except:
                error += 100
    return error,
...
for run in range(num_runs):
    ...
    plt.title("x*x*x*x+4*x*x+7*x+11")
...
if num_runs-solutions_found_cs>0:
    avg_deviations=avg_deviations_cs/(num_runs-solutions_found_cs)

if solutions_found_sta_gp>0:
    avg_generations_sta_gp/=solutions_found_sta_gp
if num_runs-solutions_found_sta_gp>0:
    avg_deviations=avg_deviations_sta_gp/(num_runs-solutions_found_sta_gp)
